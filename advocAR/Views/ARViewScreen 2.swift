...
//        NavigationView {
        ZStack(alignment: .bottom) {
            ...
            if self.isPlacementEnabled {
                ...
                /* Passing $ sign because isPlacementEnable is binding variable, binding variable is a property that has its
                 source of truth outside of the model picker view (in our case it is in main content view).
                 So, by adding $ sign we have read/write access to the variable, if we dont add $ sign, then we just have read access. */
